...
class SpellChecker(object):
    ...
    def candidates(self, word: KeyT) -> typing.Set[str]:
        ...
        if self._distance == 2:
            ...
            if tmp:
                return tmp
        if self._distance == 3:
            tmp = self.known([x for x in self.__edit_distance_3(res)])
            if tmp:
                return tmp
        return {word}
    ...
    def unknown(self, words: typing.Iterable[KeyT]) -> typing.Set[str]:
        ...
        return list(w for w in tmp if w not in self._word_frequency.dictionary)
    ...
    def __edit_distance_3(self, words: typing.Iterable[KeyT]) -> typing.List[str]:
        """Compute all strings that are 1 edits away from all the words using
        only the letters in the corpus
        ...
        tmp_words = [ensure_unicode(w) for w in words]
        tmp = [w if self._case_sensitive else w.lower() for w in tmp_words if self._check_if_should_check(w)]
        return [e3 for e2 in tmp for e3 in self.known(self.edit_distance_2(e2))]
